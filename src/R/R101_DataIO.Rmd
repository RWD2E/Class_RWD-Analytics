...
First, will need to setup ODBC connector object (i.e. Data Source Name, or, DSN). You will need to grab a couple of snowflake configuration variables from your Snowflake account; 
1. account = {Account/Server URL}
2. user = {...@umsystem.edu}
3. role = {Role}
4. authenticator = "externalbrowser"
5. database = "CLASS_MEMBER_BBME8550_STUDENTS_DB"
6. warehouse = "NEXTGENBMI_WH"
For consistency with codes below, we will name this DSN `snowflake_deid`

Then, use `DBI:dbConnect` and `odbc::odbc()` function with stored credentials to make database connection. Due to re-direction to Umsystem log-in page with the Shibboleth method, you may not be able to Knit this report at once. 
...
dat1a<-dbGetQuery(
    ...
    "select * from CLASS_MEMBER_BBME8550_STUDENTS_DB.PUBLIC.ALS_RILUZOLE_DEATH_FINAL"  # sql codes
)
```
...
select * from CLASS_MEMBER_BBME8550_STUDENTS_DB.PUBLIC.ALS_RILUZOLE_DEATH_FINAL
```
...
```{r}
saveRDS(dat1a,file="C:/proj/data/als_aset.rda")
```

Then, how do we read the .rda file back into R?

```{r}
dat1_rds<-readRDS("C:/proj/data/als_aset.rda")
```

## save dataframe as CSV file (.csv)
```{r}
write.csv(dat1a,file="C:/proj/data/als_aset.csv")
```

Then, how do we read the .csv file back into R?

```{r}
dat1_csv<-read.csv("C:/proj/data/als_aset.csv")
```

## save dataframe as delimiter separated file (.txt)
```{r}
write.table(dat1a,file="C:/proj/data/als_aset.txt",sep="|")
```

Then, how do we read the .csv file back into R?

```{r}
dat1_txt<-read.table("C:/proj/data/als_aset.txt",sep="|")
```
